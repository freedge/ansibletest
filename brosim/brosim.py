...
class Brocade(resource.Resource):
    ...
    def render_PATCH(self, request):
        ...
        print("PATCH", request.getHeader('authorization'), request.uri, content)
        res=dict(Response={
        })
    ...
    def render_GET(self, request):
        content = request.content.read().decode("utf-8")
        print("GET", request.getHeader('authorization'), request.uri, content)
    ...
    def render_POST(self, request):
        ...
        if request.uri == b"/rest/login":
            ...
            print("POST", "login", request.uri, content)
            CONNECTION_ID += 1
            request.responseHeaders.addRawHeader("Authorization", "CNX" + str(CONNECTION_ID))
        else:
            print("POST", request.getHeader('authorization'), request.uri, content)
        return b"<html>Hello, world!</html>"
...
class Svc(resource.Resource):
    ...
    def render_POST(self, request):
        ...
        if request.uri == b"/rest/auth":
            time.sleep(0.1) # this can take time
            print("POST", "auth", request.uri, content)
            CONNECTION_ID += 1
            b = json.dumps({"token": "CNX" + str(CONNECTION_ID)})
            print("return:", b)
            return b.encode('utf-8')
        elif request.uri == b'/rest/lshost/test1':
            print("POST", request.getHeader('x-auth-token'), request.uri, content)
